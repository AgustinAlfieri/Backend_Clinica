...
const dni = Joi.string().pattern(/^[0-9]{8}[A-Za-z]$/).messages({
...
const specialty = Joi.string().min(3).max(50);

const insuranceNumber = Joi.string().alphanum().min(5).max(20);

const medicalInsuranceId = Joi.string().uuid();

const medicalSpecialtyIds = Joi.array().items(Joi.string().uuid());

const appointmentsIds = Joi.array().items(Joi.string().uuid());
