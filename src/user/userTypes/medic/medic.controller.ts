...
async function findAll(req: Request, res: Response) {
  em.find(Medic, {}, { populate: ['medicalSpecialty', 'appointments'] }).then(
    (medics) => {
...
async function create(req: Request, res: Response) {
  const { dni, name, email, password, telephone, license, m_specialty } = req.body.sanitizedInput;

  const specialty = await em.findOneOrFail(MedicalSpecialty, m_specialty).then((__specialty) => {
    if (!__specialty) throw new AppError('Especialidad no encontrada', StatusCodes.NOT_FOUND);
    return __specialty;
  });
  ...
  await em
    .flush()
    .then(() => {
      res.status(StatusCodes.CREATED).send(medic);
    })
    .catch(() => {
      throw new AppError('Error en busqueda', StatusCodes.INTERNAL_SERVER_ERROR);
    });
...
async function update(req: Request, res: Response) {
  ...
  const medic = await em
    .findOneOrFail(Medic, { id })
    .then((med) => {
      return med;
    })
    .catch(() => {
