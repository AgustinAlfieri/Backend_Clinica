...
export class MedicService {
    ...
    async create(medic: MedicDTO) : Promise<Medic> {
        ...
        (medic.telephone) ? 
            newMedic.telephone = medic.telephone : newMedic.telephone = undefined;


        newMedic.medicalSpecialty = new Collection<MedicalSpecialty>(newMedic);
        if(!(medic.medicalSpecialty === null) && medic.medicalSpecialty.length > 0) {
            for (const ms of medic.medicalSpecialty) {
                const id = ms.id;
